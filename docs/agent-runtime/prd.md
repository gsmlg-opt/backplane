...
**Date:** 2026-09-10  
**Status:** Proposed; implementation and consumer verification required  
...
Supply an opt-in set of runtime collaboration tools. Supply file/resource, command, plan, and backend-dependent memory/Skill tools in a separate optional package. Tool availability is not tool authorization.
...
The current user requirements supersede the older singleton-agent target in the attached `Repo Analysis Request.txt`. That attachment remains historical context for restricted autonomous profiles and host-level heartbeat/reflection/schedule triggers. It does not require a singleton runtime, Oban, or a particular database.

Repository observations are from earlier static excerpts, not a new current-branch review. See the source register in [design.md](design.md). Shared AI/Skill package names describe adjacent proposed boundaries; availability and versions must be verified before implementation.
...
The intended value is a shared enforcement boundary and reusable runtime primitives, not a superficial common API over three independent executors. A fix to stale-result fencing, nested cancellation, tool-loop termination, or root accounting should be reusable across all migrated paths.
...
A clean Mix application installs the released package and runs scripted model/tool tasks without Backplane service applications. A consumer chooses its own adapters and runtime namespace. Test-only libraries, example servers, and service configuration do not enter production dependencies.
...
| M1 — Shared execution core | Independent package, deterministic kernel, context ownership, policy/tool gateway, provider effects, budgets, cancellation, and ephemeral execution. |
| M2 — Agent collaboration and tools | Managed/embedded hosts, nested subagents, peer delegation, bounded messaging/waits, built-in collaboration tools, optional basic tool families. |
...
| M5 — Release readiness | Standalone artifact checks, cross-consumer acceptance, compatibility/rollback documentation, and removal of migrated duplicate enforcement paths. |
...
Runtime-supplied, opt-in tools: `agent_discover`, `agent_spawn`, `agent_delegate`, `agent_send`, `run_status`, `run_wait`, `run_cancel`, and `ask_user`.

Optional package V1 tools: `file_read`, `list_dir`, `glob`, `grep`, `file_write`, `file_edit`, `exec`, `job_read`, `job_cancel`, `plan_read`, and `plan_update`.

Backend-conditional bridges: `memory_search`, `memory_store`, `skill_list`, and `skill_load`. Their wrapper contracts are in scope; building a new memory backend or Skill distribution service is not. Without a suitable configured port, those tools are not advertised.
...
| --- | --- | --- |
| FR-01 | Runtime is independently consumable without session, Git, shell, Phoenix, Backplane server, database, or optional tools. Tools package depends on runtime, never the reverse. | AC-01 |
...
| FR-18 | Direct calls, built-ins, optional tools, product tools, and MCP adapters use one registered-tool gateway and effect lifecycle. No arbitrary module/function dispatch from model input. | AC-18 |
...
| FR-20 | Optional resource tools use authorized namespaces and expected-revision/create-only writes. Traversal, symlink/race, output, and backend-confinement guarantees are explicit and tested. | AC-20 |
| FR-21 | Optional commands are run-owned, explicitly authorized, deadline/output bounded, environment restricted, and cancellable through tested backend/platform cleanup. No implicit detached jobs or sandbox claim. | AC-21 |
| FR-22 | Optional plans are task-scoped revisioned resources. Plan edits do not silently schedule tasks or overwrite newer versions. | AC-22 |
| FR-23 | Memory/Skill bridges require configured ports. Scope and provenance are enforced; Skill declarations do not authorize tools or expose another agent's memory. | AC-23 |
...
| FR-33 | Reuse verified sibling provider/Skill contracts. No copied provider parser, compulsory memory/Skill service, test-only production dependency, or unverified package-version assumption. | AC-33 |
| FR-34 | Tool installation, agent registration, and per-task invocation permission are independent. Empty tools and restrictive role/task profiles work; unavailable backends do not advertise unusable tools. | AC-34 |
...
| --- | --- |
| NFR-01 — Standalone installation | Build release artifacts in fresh Mix consumers, with Backplane service apps absent. Test runtime alone and runtime plus tools. Confirm release contents and production dependency tree. |
...
| NFR-06 — Compatibility | Baseline records pin exact tested consumer SHAs, package versions, Elixir/OTP versions, platform capabilities, and public behaviour fixtures. Unsupported configurations return explicit errors. |
...
| ID | Given / when / then |
| --- | --- |
| AC-01 | In a fresh Mix consumer, run a scripted model interaction with no optional tools or service apps. It completes without repository/session/environment requirements. Installing optional tools does not change enabled tools. |
...
| AC-18 | Exercise the same denied operation through a direct API, collaboration wrapper, optional tool, host adapter, and MCP adapter fixture. All stop at the same policy gateway before side effects. |
...
| AC-23 | Omit MemoryPort/SkillPort and verify the tools are absent. With fake ports configured, cross-agent memory is denied and Skill content does not activate unauthorized tools. |
...
| AC-33 | Verify pinned sibling types/dependencies and absence of duplicate provider codecs. Production package inspection excludes test-support servers/libraries and starts no Skill/memory/service backend. |
| AC-34 | Use distinct Planner/Coder/Reviewer/service profiles plus a zero-tool agent. Tool descriptors and effective per-task grants differ, unavailable backends are omitted, and a profile name cannot expand caller authority. |
...
- Fresh-consumer artifact builds, runtime-only and runtime-plus-tools boot tests, production dependency/contents inspection, and no Backplane service startup leakage.
...
| Basic tools expand attack surface | Opt-in registrations, revision-aware resource access, explicit command/sandbox capabilities, and host-owned authorization. |
...
Implementers may refine private module decomposition without changing these requirements. Changes to lifecycle ownership, authorization, context concurrency, durability promises, tool surface, public event semantics, sibling package boundaries, or V1 consumer coverage require a recorded design decision and updates to all three documents and acceptance mappings.

Do not use this task to rewrite unrelated UI, add product workflows, replace consumer databases, configure live autonomous mutations, publish packages, or deploy services without explicit authorization. The implementation plan provides bounded change sets and stop conditions.
