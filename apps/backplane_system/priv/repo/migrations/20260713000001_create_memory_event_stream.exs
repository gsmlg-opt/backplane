defmodule Backplane.Repo.Migrations.CreateMemoryEventStream do
  ...
  def up do
    ...
    execute(
      "CREATE INDEX bpm_events_project_occurred_at_idx ON #{qualified("bpm_events")} (project ASC, occurred_at DESC)",
      "DROP INDEX #{qualified("bpm_events_project_occurred_at_idx")}"
    )

    execute(
      "CREATE INDEX bpm_events_event_type_occurred_at_idx ON #{qualified("bpm_events")} (event_type ASC, occurred_at DESC)",
      "DROP INDEX #{qualified("bpm_events_event_type_occurred_at_idx")}"
    )
  end
