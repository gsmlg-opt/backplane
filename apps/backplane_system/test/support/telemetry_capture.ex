defmodule Backplane.TelemetryCapture do
  @moduledoc false
  ...
  Local to `:backplane_system` tests. Do not depend on other apps' test support.
  """
  @spec attach(pid(), [:telemetry.event_name()]) :: String.t()
  def attach(owner, events) when is_pid(owner) and is_list(events) do
    handler_id = "backplane-telemetry-capture-#{System.unique_integer([:positive])}"
