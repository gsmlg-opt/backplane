defmodule Backplane.Memory.Service do
  ...
  def read_resource("memory://graph/stats") do
    {:ok, Jason.encode!(Backplane.Memory.Graph.stats())}
  end
  ...
  def read_resource("memory://sessions/active") do
    ...
    sessions =
      repo.all(from(s in Backplane.Memory.Observations.Session, where: is_nil(s.ended_at)))
  ...
  def read_resource("memory://audit/recent") do
    entries = Backplane.Memory.Audit.list(limit: 50)
  ...
  def handle_stats(_args), do: {:ok, %{stats: Memories.stats()}}
  ...
  def handle_profile_refresh(%{"project" => project}) when is_binary(project) do
    Backplane.Memory.Workers.ProfileBuildWorker.enqueue(project)
  ...
  def handle_team_share(%{"memory_id" => memory_id, "team_id" => team_id})
    ...
    case Memories.team_share(memory_id, team_id) do
  ...
  def handle_team_feed(%{"team_id" => team_id} = args) when is_binary(team_id) do
    limit = args["limit"] || 20
    rows = Memories.team_feed(team_id, limit)
  ...
  def handle_sessions(args) when is_map(args) do
    ...
    q =
      from(s in Backplane.Memory.Observations.Session,
  ...
  def handle_patterns(args) when is_map(args) do
    ...
    q =
      from(o in Backplane.Memory.Observations.Observation,
  ...
  def handle_export(args) when is_map(args) do
    scope = args["scope"] || "global"
    rows = Memories.list(scope: scope, limit: 10_000)
  ...
  def handle_relations(%{"entity" => entity} = args) when is_binary(entity) do
    ...
    case Backplane.Memory.Graph.BFS.query(entity, depth) do
  ...
  def handle_compress_file(%{"file_path" => path, "agent_id" => agent_id, "host_id" => host_id})
    ...
    rows = Backplane.Memory.Observations.file_history([path], limit: 200)
  ...
  def handle_governance_delete(%{"memory_id" => memory_id} = args)
    ...
    with {:ok, _mem} <- Memories.get(memory_id) do
      ...
      if hard_delete do
        ...
        repo.delete_all(from(m in Backplane.Memory.Memories.Memory, where: m.id == ^memory_id))

        {:ok, %{memory_id: memory_id, status: "hard_deleted", actor: actor}}
      else
        case Memories.forget(memory_id) do
  ...
  def handle_graph_query(%{"entity" => entity} = args) when is_binary(entity) do
    ...
    case Backplane.Memory.Graph.BFS.query(entity, depth, relation) do
      {:ok, %{nodes: nodes, edges: edges}} ->
        {:ok,
         %{
           nodes: Enum.map(nodes, &Map.take(&1, [:id, :name, :type])),
  ...
  def handle_graph_stats(_args) do
    {:ok, Backplane.Memory.Graph.stats()}
  end
  ...
  def handle_consolidate(%{"session_id" => session_id}) when is_binary(session_id) do
    ...
    Backplane.Memory.Workers.ProfileBuildWorker.enqueue(session_id)
  ...
  def handle_verify(%{"memory_id" => memory_id}) when is_binary(memory_id) do
    case Memories.get(memory_id) do
      {:ok, mem} ->
        {:ok,
         %{
           exists: true,
  ...
  def handle_slot_read(%{"name" => name}) when is_binary(name) do
    case Backplane.Memory.Slots.read(name) do
      {:ok, slot} ->
        {:ok,
         %{
           name: slot.name,
  ...
  def handle_slot_write(%{"name" => name, "content" => content} = args)
    ...
    case Backplane.Memory.Slots.write(name, content, updated_by) do
      {:ok, slot} ->
  ...
  def handle_slot_list(_args) do
    slots = Backplane.Memory.Slots.list()
  ...
  def handle_enrich(%{"memory_id" => memory_id} = args) when is_binary(memory_id) do
    ...
    case repo.get(Backplane.Memory.Memories.Memory, memory_id) do
      nil ->
      ...
      mem ->
        ...
        {1, _} =
          repo.update_all(
            from(m in Backplane.Memory.Memories.Memory, where: m.id == ^memory_id),
