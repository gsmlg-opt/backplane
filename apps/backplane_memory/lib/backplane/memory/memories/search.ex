defmodule Backplane.Memory.Memories.Search do
  @moduledoc """
    ...
    with {:ok, [vector]} <- embed_fn.([query], :query, []) do
      ...
      writeback_fn =
        Keyword.get(
          opts,
          :writeback_fn,
          &Backplane.Memory.Workers.AccessWritebackWorker.enqueue/1
        )
  ...
  def hybrid_recall(query, opts \\ []) when is_binary(query) do
    ...
    writeback_fn =
      Keyword.get(opts, :writeback_fn, &Backplane.Memory.Workers.AccessWritebackWorker.enqueue/1)
