#!/usr/bin/env bash
# install with: chmod +x post-tool-use.sh
# Claude Code hook: PostToolUse (success)
# Records successful tool result as an observation.
...
out=d.get('tool_response','') or d.get('output','') or d.get('result','')
if isinstance(out,(dict,list)): out=json.dumps(out)
print(str(out)[:2000])
" "$INPUT" 2>/dev/null || true)"

if [ -n "$SESSION_ID" ] && [ -n "$CONTENT" ]; then
  PAYLOAD="$(python3 -c "import sys,json; print(json.dumps({'session_id':sys.argv[1],'content':sys.argv[2],'tool_name':sys.argv[3],'is_error':False}))" "$SESSION_ID" "$CONTENT" "$TOOL_NAME" 2>/dev/null || true)"
  if [ -n "$PAYLOAD" ]; then
