defmodule Backplane.Memory.Events.PreparationTest do
  ...
  test "prepare returns a recursively sanitized and bounded Event" do
  ...
  test "prepare redacts camel-case sensitive keys and nested header pair arrays" do
  ...
  test "prepare redacts values when sensitive keys themselves contain assignments" do
  ...
  test "prepare rejects invalid UTF-8 content and nested payload strings without raising" do
    invalid = <<255>>
  ...
  test "prepare rejects invalid attributes before persistence" do
    assert {:error, :missing_identity} =
             Preparation.prepare(%{event_type: "legacy.observation"})
  ...
  test "prepare_batch preserves order and stops at the first invalid item" do
