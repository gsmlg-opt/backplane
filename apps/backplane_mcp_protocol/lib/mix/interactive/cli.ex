defmodule Mix.Interactive.CLI do
  ...
  def main(args) do
    ...
    case transport do
      ...
      _ ->
        ...
        Usage: backplane-mcp-protocol --transport [sse|websocket|stdio|streamable_http] [options]
  ...
  defp run_sse_interactive(opts) do
    ...
    client_info = %{"name" => "Backplane.McpProtocol.CLI.SSE", "version" => "0.1.0"}
    name = SSETest
    transport = SSETest.Transport

    opts = [name: name, transport: {:sse, server_options}, client_info: client_info]
    {:ok, _} = Backplane.McpProtocol.Client.Supervisor.start_link(opts)
  ...
  defp run_websocket_interactive(opts) do
    ...
    client_info = %{"name" => "Backplane.McpProtocol.CLI.Websocket", "version" => "0.1.0"}
  ...
  defp run_stdio_interactive(opts) do
    ...
    client_info = %{"name" => "Backplane.McpProtocol.CLI.STDIO", "versoin" => "0.1.0"}
    name = STDIOTest
  ...
  defp run_streamable_http_interactive(opts) do
    ...
    client_info = %{"name" => "Backplane.McpProtocol.CLI.StreamableHTTP", "version" => "0.1.0"}
  ...
  def check_client_connection(client, attempt) do
    ...
    if cap = Backplane.McpProtocol.Client.get_server_capabilities(client) do
  ...
  def show_help do
    ...
    #{colors.info}EXAMPLES:#{colors.reset}
      ...
      backplane-mcp-protocol 

      # Connect to a remote SSE server
      backplane-mcp-protocol --transport sse --base-url https://remote-server.example.com

      # Connect to a WebSocket server
      backplane-mcp-protocol --transport websocket --base-url http://localhost:8000 --ws-path /mcp/ws

      # Connect to a StreamableHTTP server
      backplane-mcp-protocol --transport streamable_http --base-url http://localhost:8000

      # Run a local MCP server with stdio
      backplane-mcp-protocol --transport stdio --command ./my-mcp-server --args arg1,arg2
