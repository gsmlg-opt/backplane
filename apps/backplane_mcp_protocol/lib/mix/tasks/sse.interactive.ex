defmodule Mix.Tasks.Backplane.McpProtocol.Sse.Interactive do
  ...
  >
  > For testing new servers, please use `mix backplane.mcp_protocol.streamable_http.interactive` instead.
  ...
  * `--header` - Add a header to requests (can be specified multiple times)
  ...
  ## Examples
      ...
      mix backplane.mcp_protocol.sse.interactive --base-url http://localhost:8000
      mix backplane.mcp_protocol.sse.interactive --header "Authorization: Bearer token123"
      mix backplane.mcp_protocol.sse.interactive --header "X-API-Key: secret" --header "X-Custom: value"
  """
  ...
  def run(args) do
    ...
    Application.ensure_all_started([:backplane_mcp_protocol, :peri])
