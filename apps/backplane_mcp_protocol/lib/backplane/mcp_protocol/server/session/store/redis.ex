if Code.ensure_loaded?(Redix) do
  defmodule Backplane.McpProtocol.Server.Session.Store.Redis do
    @moduledoc """
    ...
    @behaviour Backplane.McpProtocol.Server.Session.Store

    use GenServer
    use Backplane.McpProtocol.Logging

    alias Backplane.McpProtocol.Server.Session.Store

    # 30 minutes in milliseconds
    @default_ttl 1_800_000
    @default_namespace "backplane_mcp_protocol:sessions"
    ...
    def init(opts) do
      ...
      conn_name = Keyword.get(opts, :connection_name, :backplane_mcp_protocol_redis)
      ...
      # Start Redix connection pool with backplane_mcp_protocol_ prefix to avoid conflicts
      children =
        for i <- 1..pool_size do
          child_id = :"backplane_mcp_protocol_#{conn_name}_#{i}"
      ...
      # Use backplane_mcp_protocol_ prefix for supervisor name
      supervisor_name = :"backplane_mcp_protocol_#{conn_name}_supervisor"
    ...
    defp get_connection(state) when is_struct(state, State) do
      ...
      :"backplane_mcp_protocol_#{state.conn_name}_#{index}"
    end
