defmodule Backplane.McpProtocol.Server.TaskStore do
  @moduledoc """
  Behaviour for pluggable MCP task storage backends.

  A TaskStore tracks `Backplane.McpProtocol.Server.Task` entries scoped to a session.
  Adapters are wired via the `:task_store` option of `Backplane.McpProtocol.Server.Supervisor`
  using the same `{module, opts}` shape as `:registry` and `:supervisor`:

      {Backplane.McpProtocol.Server, transport: :stdio, task_store: {MyApp.HordeTaskStore, []}}

  Phase 1 ships with the in-memory `Backplane.McpProtocol.Server.TaskStore.Local` adapter.
  ...
  supervision tree using `Backplane.McpProtocol.Server.Registry.task_store_name/1`) or expose
  ...
  alias Backplane.McpProtocol.Server.Task
  ...
  Defaults to `Backplane.McpProtocol.Server.Registry.task_store_name(server)` when not
  ...
  def resolve_name(adapter, server, opts) do
    if Code.ensure_loaded?(adapter) and function_exported?(adapter, :resolve_name, 2) do
      ...
      Backplane.McpProtocol.Server.Registry.task_store_name(server)
    end
  end
end
