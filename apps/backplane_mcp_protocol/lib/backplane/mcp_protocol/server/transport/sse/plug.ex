if Code.ensure_loaded?(Plug) do
  defmodule Backplane.McpProtocol.Server.Transport.SSE.Plug do
    @moduledoc """
    ...
    > of the Streamable HTTP transport (`Backplane.McpProtocol.Server.Transport.StreamableHTTP.Plug`).
    >
    ...
    >
    > For new implementations, please use `Backplane.McpProtocol.Server.Transport.StreamableHTTP.Plug` instead.
    ...
    ## Usage in Phoenix Router
        ...
        scope "/mcp" do
          ...
          # SSE endpoint
          get "/sse", Backplane.McpProtocol.Server.Transport.SSE.Plug,
            server: :your_server_name, mode: :sse

          # POST endpoint
          post "/messages", Backplane.McpProtocol.Server.Transport.SSE.Plug,
    ...
    ## Usage in Plug Router (Standalone)
        ...
        plug Backplane.McpProtocol.Server.Transport.SSE.Plug,
    ...
    - `:registry` - The registry to use. See `Backplane.McpProtocol.Server.Registry.Adapter` for more information (default: Elixir's Registry implementation)
    ...
    alias Backplane.McpProtocol.Server.Transport.SSE
    alias Backplane.McpProtocol.SSE.Streaming
    alias Backplane.McpProtocol.Telemetry
    alias Plug.Conn.Unfetched

    @deprecated "Use Backplane.McpProtocol.Server.Transport.StreamableHTTP.Plug instead"
    ...
    def call(conn, %{mode: :sse} = opts) do
      opts = resolve_authorization(opts)
      ...
      if conn.request_path == "/.well-known/oauth-protected-resource" do
        ...
        case authorize(conn, opts) do
          {:ok, conn, claims} ->
            handle_sse_endpoint(conn, Map.put(opts, :auth_claims, claims))
    ...
    def call(conn, %{mode: :post} = opts) do
      ...
      case authorize(conn, opts) do
        {:ok, conn, claims} ->
          handle_post_endpoint(conn, Map.put(opts, :auth_claims, claims))
    ...
    defp resolve_authorization(%{server: server} = opts) do
      auth_config = ServerSupervisor.get_authorization_config(server)
      Map.put(opts, :authorization, auth_config)
    end
    ...
    defp validate_bearer_token(conn, token, auth_config) do
      {validator_mod, _validator_opts} = auth_config.validator
      ...
      case validator_mod.validate_token(token, auth_config) do
        {:ok, raw_claims} ->
          ...
          with :ok <- Authorization.validate_expiry(claims),
            ...
            {:error, :token_expired} ->
              send_auth_error(conn, auth_config, 401)
    ...
    defp send_auth_error(conn, auth_config, 401) do
      www_auth = Authorization.build_www_authenticate(auth_config, :unauthorized)
