defmodule Backplane.McpProtocol.MCP.Message do
  @moduledoc """
  ...
  @elicitation_create_params %{
    ...
    "requestedSchema" => {:required, {:custom, &Backplane.McpProtocol.MCP.ElicitationSchema.validate/1}}
  }
  ...
  **Deprecated**: Prefer using `encode_progress_notification/2` with a params map.
  ...
  Delegates to the version module via `Backplane.McpProtocol.Protocol.Registry`.
  ...
  def progress_params_schema_for(version) do
    Backplane.McpProtocol.Protocol.Registry.progress_params_schema(version)
  end
