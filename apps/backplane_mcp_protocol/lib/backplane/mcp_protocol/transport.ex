defmodule Backplane.McpProtocol.Transport do
  @moduledoc """
  Functional behaviour for MCP transport implementations.

  Unlike `Backplane.McpProtocol.Transport.Behaviour` (which defines a GenServer-oriented transport
  ...
  - `Backplane.McpProtocol.Transport.STDIO` — newline-delimited JSON over stdin/stdout (client)
  - `Backplane.McpProtocol.Transport.StreamableHTTP` — JSON over HTTP request/response bodies (client)
  - `Backplane.McpProtocol.Transport.SSE` — JSON wrapped in SSE event format (client)
