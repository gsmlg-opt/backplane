defmodule Backplane.McpProtocol.Client.Supervisor do
  @moduledoc false
  ...
  alias Backplane.McpProtocol.Transport.Websocket
  ...
  ## Arguments
    ...
    * `opts` - Supervisor options including:
      * `:name` - Optional custom name for the client process (defaults to `Backplane.McpProtocol.Client`)
  ...
  ## Examples
      ...
      Backplane.McpProtocol.Client.Supervisor.start_link(
  ...
  def init(opts) do
    ...
    client_info =
      Keyword.get(opts, :client_info) ||
        raise ArgumentError, """
        :client_info is required when starting Backplane.McpProtocol.Client.
