...
Let's explore patterns we've discovered while building with Backplane.McpProtocol MCP. What challenges are you facing?
...
defmodule MyApp.LiveDataServer do
  ...
  def handle_info({:data_update, _data}, frame) do
    Backplane.McpProtocol.Server.send_resources_list_changed()
    {:noreply, frame}
  end
end
```
...
defmodule MyApp.ComponentTest do
  ...
  describe "complex tool validation" do
    test "validates required fields" do
      ...
      assert {:error, errors} =
        Backplane.McpProtocol.Server.Component.validate_params(tool, %{})
    ...
    test "executes with valid params" do
      ...
      frame = %Backplane.McpProtocol.Server.Frame{assigns: %{user: %{id: 1}}}

      assert {:reply, %Backplane.McpProtocol.Server.Response{} = response, ^frame} =
...
defmodule MyApp.LoggingServer do
  ...
  def handle_tool_call("logged_tool", %{input: input}, frame) do
    Backplane.McpProtocol.Server.send_log_message(:info, "Processing tool: logged_tool")

    result = process_input(input)

    Backplane.McpProtocol.Server.send_log_message(:debug, "Tool completed: logged_tool")
...
Control Backplane.McpProtocol's internal logging verbosity:

```elixir
# In config/config.exs
config :backplane_mcp_protocol, :logging,
...
config :backplane_mcp_protocol, log: false
```
