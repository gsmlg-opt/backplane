...
```elixir
Mix.install([{:backplane_mcp_protocol, "~> 1.6.3"}]) # x-release-please-version
...
```bash
mix backplane.mcp_protocol.stdio.interactive -c elixir --args=--no-halt,my_app.exs
```
...
How do AI assistants know what your components do? They read descriptions. Backplane.McpProtocol makes this simple.
...
defmodule MyAppWeb.Endpoint do
  ...
  plug Backplane.McpProtocol.Server.Transport.StreamableHTTP.Plug,
    server: MyApp.Server,
    path: "/mcp"
...
Backplane.McpProtocol automatically formats your error responses according to the MCP protocol.
...
Backplane.McpProtocol provides interactive `Mix` tasks for different transports if you need quick testing:

```bash
# Test STDIO server
mix backplane.mcp_protocol.stdio.interactive --command elixir --args=--no-halt,my_app.exs

# Test HTTP server
mix backplane.mcp_protocol.streamable_http.interactive --base-url=http://localhost:8080 --header 'authorization: Bearer 123'

# With verbose logging
mix backplane.mcp_protocol.streamable_http.interactive --base-url=http://localhost:4000 -vvv
```
