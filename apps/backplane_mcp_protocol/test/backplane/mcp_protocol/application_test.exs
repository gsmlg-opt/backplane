defmodule Backplane.McpProtocol.ApplicationTest do
  ...
  test "does not log warning when session store is disabled" do
    ...
    log =
      capture_log(fn ->
        assert [] == Backplane.McpProtocol.Application.session_store_children(config)
      end)
  ...
  test "returns child spec when session store is enabled and adapter is available" do
    config = [enabled: true, adapter: MockSessionStore, ttl: 1_800_000, namespace: "backplane_mcp_protocol:sessions"]

    assert [{MockSessionStore, config}] == Backplane.McpProtocol.Application.session_store_children(config)
  end
end
