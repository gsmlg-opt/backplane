defmodule Backplane.McpProtocol.Server.Transport.StreamableHTTP.PlugPersistenceTest do
  ...
  alias Backplane.McpProtocol.Test.MockSessionStore
  ...
  setup do
    ...
    original_config = Application.get_env(:backplane_mcp_protocol, :session_store)

    Application.put_env(:backplane_mcp_protocol, :session_store,
      enabled: true,
    ...
    on_exit(fn ->
      if original_config do
        Application.put_env(:backplane_mcp_protocol, :session_store, original_config)
      else
        Application.delete_env(:backplane_mcp_protocol, :session_store)
      end
    end)
  ...
  describe "session lifecycle with persistence" do
    ...
    test "initialize request triggers session persistence", ctx do
      ...
      {:ok, session} =
        start_supervised(
          {Backplane.McpProtocol.Server.Session,
