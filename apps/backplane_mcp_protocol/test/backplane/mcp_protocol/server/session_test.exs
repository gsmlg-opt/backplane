defmodule Backplane.McpProtocol.Server.SessionTest do
  ...
  describe "send_notification/3" do
    ...
    test "sends notification to transport", %{server: session} do
      assert :ok =
               :info
               |> Backplane.McpProtocol.Server.send_log_message("hello")
               |> then(fn _ ->
                 send(
                   session,
  ...
  describe "send_resource_updated subscription gating" do
    ...
    test "Server.send_resource_updated/2 routes through the gate", %{
      ...
      :sys.replace_state(session, fn state ->
        ...
        Backplane.McpProtocol.Server.send_resource_updated("file:///watched")
  ...
  describe "elicitation requests" do
    ...
    test "send_elicitation_request rejects invalid schema synchronously" do
      ...
      assert {:error, _} = Backplane.McpProtocol.Server.send_elicitation_request("hi", bad_schema)
    end
  end
end
