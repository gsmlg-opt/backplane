defmodule Backplane.McpProtocol.Server.Authorization.PlugIntegrationTest do
  ...
  alias Backplane.McpProtocol.Server.Transport.StreamableHTTP.Plug, as: SHTTPPlug
  ...
  defp setup_auth_config(_context) do
    :persistent_term.put({Backplane.McpProtocol.Server.Supervisor, FakeServer, :authorization_config}, auth_config())

    :persistent_term.put(
      {Backplane.McpProtocol.Server.Supervisor, FakeServer, :session_config},
      %{
        registry_mod: Local,
        task_supervisor: nil
      }
    ...
    on_exit(fn ->
      :persistent_term.erase({Backplane.McpProtocol.Server.Supervisor, FakeServer, :authorization_config})
      :persistent_term.erase({Backplane.McpProtocol.Server.Supervisor, FakeServer, :session_config})
    end)
  ...
  describe "no authorization configured" do
    setup do
      ...
      :persistent_term.put(
        {Backplane.McpProtocol.Server.Supervisor, FakeServer, :session_config},
        %{
          registry_mod: Backplane.McpProtocol.Server.Registry.None,
          task_supervisor: nil
        }
      )

      :persistent_term.put(
        {Backplane.McpProtocol.Server.Supervisor, FakeServer, :session_supervisor_mod},
        DynamicSupervisor
      )

      on_exit(fn ->
        :persistent_term.erase({Backplane.McpProtocol.Server.Supervisor, FakeServer, :session_config})
        :persistent_term.erase({Backplane.McpProtocol.Server.Supervisor, FakeServer, :session_supervisor_mod})
      end)
  ...
  describe "Backplane.McpProtocol.Server.Transport.WellKnown plug" do
    alias Backplane.McpProtocol.Server.Transport.WellKnown

    setup do
      config = auth_config()
      :persistent_term.put({Backplane.McpProtocol.Server.Supervisor, FakeServer, :authorization_config}, config)
