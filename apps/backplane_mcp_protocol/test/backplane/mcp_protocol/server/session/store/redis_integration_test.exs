defmodule Backplane.McpProtocol.Server.Session.Store.RedisIntegrationTest do
  use ExUnit.Case, async: false

  alias Backplane.McpProtocol.Server.Session.Store.Redis
  ...
  @redis_config [
    ...
    ttl: 10_000,
    namespace: "backplane_mcp_protocol:test",
    connection_name: :test_redis
  ]

  setup _context do
    Application.put_env(:backplane_mcp_protocol, :session_store, @redis_config)

    on_exit(fn ->
      {:ok, conn} = Redix.start_link("redis://localhost:6379")

      try do
        case Redix.command(conn, ["KEYS", "backplane_mcp_protocol:test:*"]) do
          {:ok, []} -> :ok
