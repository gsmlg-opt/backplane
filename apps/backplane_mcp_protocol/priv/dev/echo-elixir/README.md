...
A comprehensive Phoenix-based MCP server that demonstrates Server-Sent Events transport and the full spectrum of MCP capabilities through Backplane.McpProtocol.

## Exploring Real-time MCP Communication

What happens when you combine Phoenix's real-time capabilities with the Model Context Protocol? The Echo server answers this by implementing an SSE-based transport that enables streaming communication between clients and servers. This approach showcases how Backplane.McpProtocol adapts to different transport mechanisms while maintaining protocol compliance.
...
The foundational echo tool demonstrates stateful request handling. Each invocation increments a server-side counter, showing how Backplane.McpProtocol servers can maintain context across requests.
...
Notice how the server uses Backplane.McpProtocol' frame pattern for state management:
