defmodule Upcase.Server do
  ...
  use Backplane.McpProtocol.Server, capabilities: [:tools, :resources, :prompts]
  ...
  def handle_resource_read("env:///" <> variable, frame) do
    case System.get_env(variable) do
      nil ->
        {:error, Backplane.McpProtocol.MCP.Error.resource(:not_found, %{variable: variable}), frame}
  ...
  def handle_resource_read(_uri, frame) do
    {:error, Backplane.McpProtocol.MCP.Error.resource(:not_found, %{}), frame}
  end
