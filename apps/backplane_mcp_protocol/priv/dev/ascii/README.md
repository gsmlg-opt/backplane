# ASCII Art Studio with Backplane.McpProtocol MCP
...
{:ok, result} = Backplane.McpProtocol.Client.call_tool(client, "text_to_ascii", %{
  text: "HELLO",
...
{:ok, %{"fonts" => fonts}} = Backplane.McpProtocol.Client.call_tool(client, "list_fonts", %{})
```
...
{:ok, banner} = Backplane.McpProtocol.Client.call_tool(client, "generate_banner", %{
  text: "Welcome",
  width: 50
})
```
...
children = [
  Backplane.McpProtocol.Server.Registry,
...
The ASCII Art Studio demonstrates that MCP servers built with Backplane.McpProtocol integrate naturally into existing Phoenix applications, enabling you to serve both human users and AI assistants from a single, well-architected codebase.
