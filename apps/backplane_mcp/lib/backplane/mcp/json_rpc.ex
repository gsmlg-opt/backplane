defmodule Backplane.MCP.JsonRpc do
  @moduledoc """
  Backplane-local JSON-RPC envelope helpers for MCP transport code.

  Protocol version metadata and ID generation come from `backplane_mcp_protocol`; these
  helpers keep response construction as maps because the transport code sends
  both JSON and SSE-wrapped JSON.
  """
  ...
  defp request_id(opts), do: Keyword.get(opts, :id, Backplane.McpProtocol.MCP.ID.generate_request_id())
