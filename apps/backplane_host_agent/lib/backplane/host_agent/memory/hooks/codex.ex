defmodule Backplane.HostAgent.Memory.Hooks.Codex do
  @moduledoc """
  Normalizes the hook input contract exposed by Codex CLI 0.146.

  Source hooks without a parity event in the Backplane lifecycle use
  versioned `codex.*` event types rather than being silently coerced.
  """

  alias Backplane.HostAgent.Memory.EventEnvelope

  @hooks %{
    "pretooluse" => {"pre-tool-use", "codex.tool.pre_use.v1"},
    "permissionrequest" => {"permission-request", "codex.permission.requested.v1"},
    "posttooluse" => {"post-tool-use", "agent.tool.completed"},
    "precompact" => {"pre-compact", "agent.context.pre_compact"},
    "postcompact" => {"post-compact", "codex.context.post_compact.v1"},
  ...
  @doc "Lists the hook names exposed by the verified Codex 0.146 contract."
  def supported_hooks, do: @supported_hooks

  @doc "Normalizes one supported Codex hook using trusted host configuration."
  ...
  def normalize(hook, source, config) when is_binary(hook) and is_map(source) do
    with {:ok, canonical_hook, event_type} <- resolve_hook(hook),
      ...
      idempotency_key = "codex:#{session_id}:#{canonical_hook}:#{digest(source_id)}"
      ...
      {:ok, Map.reject(envelope, fn {_key, value} -> is_nil(value) end)}
    end
  end
  ...
  defp source_id(source) do
    Enum.find_value(~w(source_event_id hook_event_id tool_use_id tool_call_id turn_id), fn key ->
  ...
  defp agent_id(source, config) do
    optional_config_string(config, :agent_id) || optional_string(source, "agent_id") || "codex"
  end
